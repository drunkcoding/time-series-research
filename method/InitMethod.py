...
def merge_excel(dirct, key='date'):
    ...
    for i in range(2, len(tmp_excels)):
        tmp = pd.merge(tmp, tmp_excels[i], on=key)
    return tmp


def combine_excels(df1, df2, key='close'):
    def new_data(df, label): return pd.DataFrame(np.transpose([df.date.values, df[key].values]),
                                                 columns=['date', label])
...
def cointegration_test(s1, s2):
    ...
    def residual(param, y, x): return np.subtract(y, np.multiply(param, x))
...
def select_data(df, dic='data\\Chinese_Stock\\data_code\\'):
    ...
    for x in code_list:
        print(x)
        try:
            tmp = ts.get_k_data(x, start='2013-01-01', autype=None)
        except:
            pass
        tmp = tmp.dropna()
        tmp.to_csv(dic+x+'.csv')
...
class MAP(object):
    ...
    def generate(self, step_t):
        def make(L): return [try_t(i, L, step_t)
                             for i in range(step_t, length)]
